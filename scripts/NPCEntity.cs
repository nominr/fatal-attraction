...
public partial class NPCEntity : Area2D
{
	...
	{
		// Check proximity before allowing interaction
		if (@event is InputEventMouseButton mouseEvent && mouseEvent.Pressed && mouseEvent.ButtonIndex == MouseButton.Left)
		{
			if (_playerInRange)
			{
				GD.Print($"Click interaction with {NpcId}");
				EmitSignal(SignalName.NPCClicked, NpcId);
			}
			else
			{
				GD.Print($"Too far to interact with {NpcName}");
			}
		}
	}
	...
	private void OnBodyEntered(Node2D body)
	{
		...
		if (body.IsInGroup("players"))
		{
			...
			// Just show hint when player enters range, don't auto-interact
			// Player must click to interact
